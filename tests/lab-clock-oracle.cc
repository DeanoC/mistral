...
// Compare fixed-placement Quartus references differing only in clock polarity.
...
int main(int argc, char **argv)
{
  ...
  for(int i = 0; i != 2; ++i) {
    ...
    if(!models[i]->opt_r_set(CV::JTAG_ID, 0)) return 2;
  }
  bool lab = argc > 3 && std::string(argv[3]) == "LAB";
  if(argc > 3 && !lab && std::string(argv[3]) != "MLAB") return 2;
  int clocks = argc > 4 && std::string(argv[4]) == "3" ? 3 : 1;
  if(argc > 4 && std::string(argv[4]) != "1" && std::string(argv[4]) != "3") return 2;
  const CV::bmux_type_t inversion[] = {CV::CLK0_INV, CV::CLK1_INV, CV::CLK2_INV};
  for(int i = 0; i < clocks; ++i)
    if(!positive->bmux_b_set(lab ? CV::LAB : CV::MLAB, CV::xy2pos(lab ? 7 : 8, 32), inversion[i], 0, true)) return 2;
  ...
  std::puts("PASS: CLKx_INV exactly reproduces fixed-placement Quartus falling-edge configuration");
}
